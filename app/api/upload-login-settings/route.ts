            ...
            try {
                ...
                if (existingUser.rows.length === 0) {
                    ...
                    await sql`
                        ...
                        VALUES (${examNo}, ${crypto.createHash('sha256').update(password).digest('hex')}, ${password}, ${examNo}, 'student', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                    `;
                } else {
                    // 既存ユーザーのパスワードを更新
                    await sql`
                        UPDATE users 
                        SET password_hash = ${crypto.createHash('sha256').update(password).digest('hex')}, phone_last4 = ${password}, updated_at = CURRENT_TIMESTAMP
